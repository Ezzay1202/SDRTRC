"""
SDR-TR v3.2: TRPL-based Trust-Region Residual Learning with Frequency-Aware State

v3.2-min (Phase 1 fix — quick validation):
...
Invoke with: --model SDRTR_v31  # v3.2 --sdr_version 4
...
class TRPLResidualBranch(nn.Module):
    ...
    def __init__(self, configs):
        ...
        nn.init.constant_(self.r_net[-1].bias, -4.0)  # v3.2: init r≈0.008
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.disable_dep:
            ...
            print('[SDR-TR v3.2] dependency branch DISABLED — XLinear-only mode')
        else:
            self.dep_branch = TRPLResidualBranch(configs)
            print('[SDR-TR v3.2] TRPLResidualBranch active')
    ...
    def get_aux_loss(self, target=None):
        """v3.2: r_reg penalty to prevent radius explosion."""
